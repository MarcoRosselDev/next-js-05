...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!email || !password || !confirmPassword) {    return NextResponse.json({message: messages.error.needProps},{status: 400})}
    // validar que el el email es un email valido
    if (!comprobacion_email(email)) {                 return NextResponse.json({message: messages.error.bad_email_format},{status: 400})} // se pueden aplicar librerias aqui
    // validar que los password sean el mismo
    if (password !== confirmPassword) {               return NextResponse.json({message: messages.error.password_mistake},{status: 400})}
    ...
    if (encontrar_usuario) {                          return NextResponse.json({message: messages.error.email_existente},{status: 400})}
    ...
    const nuevo_usuario: IUserSchema =  new User({  email,    password:password_encriptado   })
    // @ts-ignore
